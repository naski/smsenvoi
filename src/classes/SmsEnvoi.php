...
* @author Stéphane Wouters <stephane@doelia.fr>
...
class SmsEnvoi
{
     ...
     * @param int    $richsms_option Option rich SMS à utiliser: 1 ou 2
    ...
    public function sendSMS($recipients, $content, $subtype = 'PREMIUM', $senderlabel = 'SMS ENVOI', $senddate = '', $sendtime = '', $richsms_option = '', $richsms_url = '')
    {
        ...
        if ($senddate != '') {
            $fields['message']['senddate'] = $senddate;
        }
        if ($sendtime != '') {
            $fields['message']['sendtime'] = $sendtime;
        }
     ...
     *
     * @param int    $cobrandingmember_id ID du compte marque blanche effectuant l'envoi
     * @param string $recipients          liste des numéros destinataires séparés par une virgule
     * @param string $content             contenu du message
     * @param string $subtype             Gamme utilisée pour l'envoi ( LOWCOST / STANDARD / PREMIUM / STOP / LONG ) paramètre optionnel, par défaut : PREMIUM
     * @param string $senderlabel         Nom d'expéditeur à afficher pour les gammes le permettant
     ...
     *
     * @param int $message_id ID du message concerné
     */
    ...
    public function createCobrandingMember($email, $motdepasse, $numero_portable = '', $nom = '', $prenom = '', $raison_sociale = '')
    {
        if ($email == '' || $motdepasse == '') {
            if ($this->debug) {
                echo "ECHEC Lors de la création du compte : L'e-mail et le mot de passe doivent être remplis";
            }
        ...
        if ($numero_portable != '') {
            $fields['cobrandingmember']['numero_portable'] = $numero_portable;
        }
        if ($nom != '') {
            $fields['cobrandingmember']['nom'] = $nom;
        }
        if ($raison_sociale != '') {
            $fields['cobrandingmember']['raison_sociale'] = $raison_sociale;
        }
     ...
     * Fonction permettant de créer un compte marque blanche ( Nécessite de créer au préalable une marque blanche SMSENVOI, 100% gratuit).
     *
     * @param int Id du compte client marque blanche
     * @param string $email           adresse e-mail de connexion du compte client marque blanche
     * @param string $motdepasse      mot de passe du compte client marque blanche
     * @param string $numero_portable (optionnel) numéro de téléphone du popriétaire du compte client marque blanche
     * @param string $nom             (optionnel) nom du propriétaire du compte client marque blanche
    ...
    public function editCobrandingMember($id, $email, $motdepasse, $numero_portable = '', $nom = '', $prenom = '', $raison_sociale = '')
    {
        if ($email == '' || $motdepasse == '' || $id == '') {
            if ($this->debug) {
                echo "ECHEC Lors de la modification du compte : L'ID, l'e-mail et le mot de passe doivent être remplis";
            }
     ...
     *
     * @param int $id Id de connexion du compte client marque blanche
     */
    public function checkCobrandingMemberInfos($id)
    {
        if ($id == '') {
            if ($this->debug) {
                echo "ECHEC, l'ID ne peut être vide";
            }
    ...
    public function checkCobrandingMemberCredits($id)
    {
        ...
        if (isset($temp['credits_sms_lowcost'])) {
            $return['LOWCOST'] = $temp['credits_sms_lowcost'];
        }
        if (isset($temp['credits_sms_standard'])) {
            $return['STANDARD'] = $temp['credits_sms_standard'];
        }
        if (isset($temp['credits_sms_premium'])) {
            $return['PREMIUM'] = $temp['credits_sms_premium'];
        }
        if (isset($temp['credits_sms_long'])) {
            $return['LONG'] = $temp['credits_sms_long'];
        }
        if (isset($temp['credits_sms_stop'])) {
            $return['STOP'] = $temp['credits_sms_stop'];
        }
     ...
     * @param int    $nb      Nombre de sms à créditer
     */
